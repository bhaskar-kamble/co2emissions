...
```{r}
path_to_data <- "D:/GITHUB_REPOS/co2emissions/Bayern/BayernData/"
```
...
```{r}
sfh_data <- main_function("SFH" , et_list , "Bayern")
mfh_data <- main_function("MFH" , et_list , "Bayern")
...
```{r}
rnk_data <- sfh_data$bundesland_data
#dim(rnk_data)
...
```{r}
rnk_data <- mfh_data$bundesland_data
#dim(rnk_data)
data.frame(table(rnk_data$abrechnungsjahr))
j <- 1
```
