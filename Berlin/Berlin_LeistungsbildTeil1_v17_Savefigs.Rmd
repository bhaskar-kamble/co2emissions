...
```{r}
b0_grad1 <- round(as.numeric(linmodel$coefficients[1]) , 2)
b1_grad1 <- round(as.numeric(linmodel$coefficients[2]) , 2)
eq_grad1 <- paste0("y = ", b1_grad1 , "x + ", b0_grad1)
ggplot(prognose_lin , aes(x=abrechnungsjahr , y = pred))+geom_point()+labs(title = "CO2 Emissionen in Berlin, alle Wohngebäude (Polynom 1. Grades)",y="kilo t.",x="Jahr"
)+annotate(geom="text",label=eq_grad1,x=2015,y=0.65*max(prognose_lin$pred),size=5) + ylim(0,max(prognose_lin$pred))+scale_x_continuous(breaks=seq(2002,2030,4))
figIndex <- 1
figIndex <- saveFigure(gtypeForFig=gtypeForFig , 
                       section = i_section,
                       subsection = i_subsection,
...
b0_grad1 <- round(as.numeric(linmodel_sfh$coefficients[1]) , 2)
b1_grad1 <- round(as.numeric(linmodel_sfh$coefficients[2]) , 2)
eq_grad1 <- paste0("y = ", b1_grad1 , "x + ", b0_grad1)

ggplot(prognose_sfh_lin , aes(x=abrechnungsjahr , y = pred))+geom_point()+labs(title="CO2 Emissionen in Berlin, 1-2 Familiengebäude (Polynom 1. Grades)",y="kilo t.",x="Jahr")+annotate(
  geom="text",label=eq_grad1,x=2015,y=0.65*max(prognose_sfh_lin$pred),size=5
  ) + ylim(0,max(prognose_sfh_lin$pred))+scale_x_continuous(breaks=seq(2002,2030,4))
...
b0_grad1 <- round(as.numeric(linmodel_mfh$coefficients[1]) , 2)
b1_grad1 <- round(as.numeric(linmodel_mfh$coefficients[2]) , 2)
eq_grad1 <- paste0("y = ", b1_grad1 , "x + ", b0_grad1)
ggplot(prognose_mfh_lin , aes(x=abrechnungsjahr , y = pred))+geom_point()+labs(title="CO2 Emissionen in Berlin, Mehrfamiliengebäude (Polynom 1. Grades)",y="kilo t.",x="Jahr"
)+annotate(
  geom="text",label=eq_grad1,x=2015,y=0.65*max(prognose_mfh_lin$pred),size=5
  ) + ylim(0,max(prognose_mfh_lin$pred))+scale_x_continuous(breaks=seq(2002,2030,4))
figIndex <- 1
figIndex <- saveFigure(gtypeForFig=gtypeForFig , 
                       section = i_section,
                       subsection = i_subsection,
