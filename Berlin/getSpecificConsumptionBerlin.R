...
getSpecificConsumptionBerlin <- function(bezirk_data , wetter_ja_nein) {
  ...
  if (wetter_ja_nein) {
    ...
    weather_data <- read.table("D:/GITHUB_REPOS/co2emissions/Berlin/berlin_wetter_tegel.txt",header=TRUE)
    weather_data$abrechnungsjahr <- 2002:2018
    names(weather_data) <- c("wind","sun","bedeckung","temperatur","abrechnungsjahr")
    weather_data <- weather_data[ , c("abrechnungsjahr","wind","sun","bedeckung","temperatur")]
