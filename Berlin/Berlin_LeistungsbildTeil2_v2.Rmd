...
The following gives area proportions for SFH buildings

```{r}
return_SFH$area_prop_table
```

The following gives area proportions for MFH buildings

```{r}
return_MFH$area_prop_table
```

Use the `areaSFH` column from the below data and multiply it with `return_SFH$area_prop_table` to get absolute area shares for SFH.

```{r}
bezirk_total_area
```

The following gives the area warmed by each ET for SFH buildings:

```{r}
areas_SFH_byET <- 0.01*bezirk_total_area$areaSFH * return_SFH$area_prop_table
areas_SFH_byET$abrechnungsjahr <- 2002:2018
areas_SFH_byET <- getRowSums(areas_SFH_byET , dropCols = "abrechnungsjahr")
```

The following gives the area warmed by each ET for MFH buildings:

```{r}
areas_MFH_byET <- 0.01*bezirk_total_area$areaMFH * return_MFH$area_prop_table
areas_MFH_byET$abrechnungsjahr <- 2002:2018
areas_MFH_byET <- getRowSums(areas_MFH_byET , dropCols = "abrechnungsjahr")
```

Now you have to add `areas_MFH_byET` and `areas_MFH_byET` to get areas of ALL buildings.
...
The following gives the area warmed by each ET for ALL buildings:

```{r}
areas_ALL_byET
#better to replace zero values by avg.
```

The following gives the absolute energy shares in kWh for each ET for all buildings:

```{r}
bezirk_aes_all #energies in kWh
#better to replace zero values by avg.
```

Now you have to divide `bezirk_aes_all` by `areas_ALL_byET` to get the specific consumption for ALL buildings and each ET. The following code does this:

```{r}
spz_vrbrch_all_byET <- bezirk_aes_all / areas_ALL_byET
spz_vrbrch_all_byET$abrechnungsjahr <- 2002:2018
spz_vrbrch_all_byET
```


Write a function to achieve the above. Needs as input: `bezirk_aes_all`, `bezirk_aes_sfh` (not yet calculated) and `bezirk_aes_mfh` (not yet calculated); areas_ALL_byET - this requires `areas_SFH_byET` and `areas_MFH_byET` - these require `bezirk_total_area$areaMFH`, `bezirk_total_area$areaSFH`, `return_MFH$area_prop_table` and `return_SFH$area_prop_table`.
...
getSpzVrbrchByET <- function(gtype,bezirk_aes,bezirk_total_area,area_prop_table_SFH,area_prop_table_MFH) {
  ...
  if (gtype=="ALL") {
    ...
    spz_vrbrch_all_byET$abrechnungsjahr <- 2002:2018
    return_data <- spz_vrbrch_all_byET
  }
