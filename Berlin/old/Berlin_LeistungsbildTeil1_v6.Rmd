...
plot_reqdColumns <- function(input_data,   # data frame
  ...
  return_object <- 
    ...
    )+labs(x=xVar,y=yVar,title=plot_title)+ylim(0,max(input_data[[yVar]]))
...
plot_reqdColumns(berlin_co2_all,
                 ...
                 yColsName = "Bezirk",
                 yVar = "kilo t.",
                 plot_title = "CO2 Emissionen von Stadtbezirken")
```
...
points_line_lm(input_data = spez_co2_emission,
               ...
               y_eq = 20,
               size_eq = 4,
               plot_title = "Spezifische CO2 Emissionen in Berlin",
               xlab = "Jahr",
               ylab = "kg/qm")
```
...
for (ii in 1:12) {
  g_co2spez_bezirk[[ii]] <- points_line_lm(input_data = bezirke_spez_co2,
                                       ...
                                       y_eq = 0.5*ymax,
                                       size_eq = 4,
                                       plot_title = bezirk_list[ii],#Here put bezirk_name[ii]
                                       xlab = "Jahr",
                                       ylab = "kg/qm")
}
require(grid)
...
ggplot(data=bezirke_spezco2_linear_2018,aes(x=bezirk,y=wert,fill=wert))+geom_bar(stat="identity")+scale_fill_gradient(
  low="green",high="red"
)+theme(legend.position="none",axis.text.x=element_text(angle=45,hjust=1))+labs(x=" ",y="kg/qm",title="Spezifische CO2 Emissionen 2018")#+coord_flip()
```
