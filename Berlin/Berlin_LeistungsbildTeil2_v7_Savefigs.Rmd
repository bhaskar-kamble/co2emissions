...
```{r}
showResults <- "hide" #options: "show" or "hide"
```
...
### `r i_section`.`r i_subsection`.`r i_subsubsection`. Stichprobengröße
...
```{r}
source("/home/kbhaskar/Github_Repos/co2emissions/Berlin/FiguresForSenate/saveFigureTeil2.R")
gtypeForFig <- "ALL"
```
...
```{r,fig.width=8.75,fig.height=6}
createTable(bezirk_sample_size$SFH,
            fontSize = 10,
            isHeader = TRUE,
            headerName = paste0(bezirk_proper_name,", 1-2  Familiengebäude, Stichprobengröße nach Energieträgern"),
            columnNames =  c("Jahr", "Erdgas","Wärme","Flüssiggas","Heizöl","Holzpellets","Strom","Gesamt"),
            columnWidths = rep(1,8),
            #columnsToRound = c("Jahr", "Erdgas","Wärme","Flüssiggas","Heizöl","Holzpellets","Strom")
)
...
```{r}
require(ggplot2)
ggplot(berlin_sample_size$MFH) + geom_point(aes(x=abrechnungsjahr,y=get(bezirk)))+labs(x="Jahr",y="Stichprobengröße",title=paste0(bezirk_proper_name , ", Mehrfamiliengebäude (Stichprobengröße)"))+scale_x_continuous(breaks=seq(2002,2018,2))
figIndex <- 1
figIndex <- saveFigureTeil2(gtypeForFig=gtypeForFig , 
                       section = i_section,
                       subsection = i_subsection,
...
```{r,fig.width=8.75,fig.height=6}
createTable(bezirk_sample_size$MFH,
            fontSize = 10,
            isHeader = TRUE,
            headerName = paste0(bezirk_proper_name,", Mehrfamiliengebäude, Stichprobengröße nach Energieträgern"),
            columnNames =  c("Jahr", "Erdgas","Wärme","Flüssiggas","Heizöl","Holzpellets","Strom","Gesamt"),
            columnWidths = rep(1,8),
            #columnsToRound = c("Jahr", "Erdgas","Wärme","Flüssiggas","Heizöl","Holzpellets","Strom")
)
...
```{r}
for (colName in names(spz_vrbrch_all_byET)) {
  spz_vrbrch_all_byET[[colName]][is.nan(spz_vrbrch_all_byET[[colName]])] <- 0.0
}
y_max <- 2.15
...
grob_gridAllETs <- function(obj,geb_type) {
  y_max <- max(obj[ , et_list])
  g_return <- list()
  ...
  require(gridExtra)
  return(
    arrangeGrob(g_return[[1]],g_return[[2]],g_return[[3]],g_return[[4]],g_return[[5]],g_return[[6]],ncol=3)
  )
}
```
...
```{r}
for (colName in names(spz_vrbrch_sfh_byET)) {
  spz_vrbrch_sfh_byET[[colName]][is.nan(spz_vrbrch_sfh_byET[[colName]])] <- 0.0
}
y_max <- 2.15
...
whatToSave <- grob_gridAllETs(spz_vrbrch_sfh_byET, geb_type = "1-2FH")
figIndex <- 1
figIndex <- saveFigureTeil2(gtypeForFig=gtypeForFig , 
                            section = i_section,
                            subsection = i_subsection,
...
```{r,fig.width=10,fig.height=6}
#spz_vrbrch_mfh_byET$fluessiggas <- 0 
for (colName in names(spz_vrbrch_mfh_byET)) {
  spz_vrbrch_mfh_byET[[colName]][is.nan(spz_vrbrch_mfh_byET[[colName]])] <- 0.0
}
plot_gridAllETs(spz_vrbrch_mfh_byET,geb_type="MFH")
whatToSave <- grob_gridAllETs(spz_vrbrch_mfh_byET, geb_type = "MFH")
figIndex <- 1
figIndex <- saveFigureTeil2(gtypeForFig=gtypeForFig , 
                            section = i_section,
                            subsection = i_subsection,
