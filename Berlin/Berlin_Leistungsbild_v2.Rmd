...
The following is simply for trying. Here I am putting simply the line plots.

```{r}

obj <- berlin_co2_all_cumsums
ggplot()+geom_line(data=obj,aes(x=abrechnungsjahr,y=get(bezirk_list[1]),color=bezirk_list[1])
...
Now I try to put the lines as well as the ribbons:
...
bezirke_spezco2_linear_2018 <- bezirke_spezco2_linear_2018[bezirke_spezco2_linear_2018$bezirk!="abrechnungsjahr" , ]
...
ggplot(data=bezirke_spezco2_linear_2018,aes(x=bezirk,y=wert))+geom_bar(stat="identity")+coord_flip()
```
...
* change with respect to previous year? i.e. 100 * [ x(i+1) - x(i) ]/ x(i) ? (i denotes the year here)
...
#barplot(bezirke_spezco2_linear_2018$wert)
ggplot(data=bezirke_spezco2_sfh_linear_2018,aes(x=bezirk,y=wert))+geom_bar(stat="identity")+coord_flip()
```
...
#barplot(bezirke_spezco2_linear_2018$wert)
ggplot(data=bezirke_spezco2_mfh_linear_2018,aes(x=bezirk,y=wert))+geom_bar(stat="identity")+coord_flip()
```
...
## `r i_section`.`r i_subsection`.	Stadtbezirke, alle Wohngebäude, Heizenergieverbrauch 2002 - 2018 summiert 
...
```{r}
plot(2002:2018 , bezirk_areas_all$total)
```
...
```{r}
plot(2002:2018 , bezirk_areas_sfh$total)
```
...
```{r}
plot(2002:2018 , bezirk_areas_mfh$total)
```
