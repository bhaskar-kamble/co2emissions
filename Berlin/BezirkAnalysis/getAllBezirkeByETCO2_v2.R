getAllBezirkeByETCO2 <- function() {
  ...
  for (i in 1:12) {
    ...
    if (i==1) {
      ...
      aes_sfh_allebezirke <- aes_SFH
      aes_mfh_allebezirke <- aes_MFH
    }
    
    if (i != 1) {
      co2_mfh_allebezirke <- co2_mfh_allebezirke + co2_MFH
      co2_sfh_allebezirke <- co2_sfh_allebezirke + co2_SFH
      aes_sfh_allebezirke <- aes_sfh_allebezirke + aes_SFH
      aes_mfh_allebezirke <- aes_mfh_allebezirke + aes_MFH
    }
  ...
  return_data[["aes_all"]] <- return_data$aes_mfh + return_data$aes_sfh
  return_data$aes_mfh$abrechnungsjahr <- 2002:2018
  return_data$aes_sfh$abrechnungsjahr <- 2002:2018
