...
path_to_file <- "D:/GITHUB_REPOS/co2emissions/Berlin/BezirkAnalysis/CO2LinearTrendForBezirke/04_SpezifischVerbrauch/"
...
for (i in 1:12) {
  ...
  SVM_SFH <- return_SFH$spz_verbrauch_mean
  SVM_MFH <- return_MFH$spz_verbrauch_mean
  ...
  SVM_SFH_lin <- linearizer(SVM_SFH , dropCols = "total" , xVar = "abrechnungsjahr")
  SVM_SFH_lin <- capZero(SVM_SFH_lin , dropCols = "abrechnungsjahr")
  SVM_MFH_lin <- linearizer(SVM_MFH , dropCols = "total" , xVar = "abrechnungsjahr")
  SVM_MFH_lin <- capZero(SVM_MFH_lin , dropCols = "abrechnungsjahr")
  
  file_name_sfh <- paste0("Spez_Verbrauch_",bezirk_number,"_",bezirk,"SFH.csv")
  file_name_mfh <- paste0("Spez_Verbrauch_",bezirk_number,"_",bezirk,"MFH.csv")
  
  write.csv2(SVM_SFH_lin,file=paste0(path_to_file , file_name_sfh) , row.names = FALSE)
  write.csv2(SVM_MFH_lin,file=paste0(path_to_file , file_name_mfh) , row.names = FALSE)
