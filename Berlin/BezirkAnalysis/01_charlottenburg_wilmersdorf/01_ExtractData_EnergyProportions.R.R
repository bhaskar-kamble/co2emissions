...
path_to_file <- "D:/GITHUB_REPOS/co2emissions/Berlin/BezirkAnalysis/CO2LinearTrendForBezirke/01_Energy_Proportions/"
...
for (i in 1:12) {
  ...
  EPT_SFH <- return_SFH$energy_prop_table
  EPT_MFH <- return_MFH$energy_prop_table
  EPT_SFH <- EPT_SFH[,c("abrechnungsjahr","erdgas","waerme","fluessiggas","heizoel","holzpellets","strom")]
  EPT_MFH <- EPT_MFH[,c("abrechnungsjahr","erdgas","waerme","fluessiggas","heizoel","holzpellets","strom")]
  
  EPT_SFH_lin <- linearizer(EPT_SFH , dropCols = "total" , xVar = "abrechnungsjahr")
  EPT_SFH_lin <- capZero(EPT_SFH_lin , dropCols = "abrechnungsjahr")
  EPT_MFH_lin <- linearizer(EPT_MFH , dropCols = "total" , xVar = "abrechnungsjahr")
  EPT_MFH_lin <- capZero(EPT_MFH_lin , dropCols = "abrechnungsjahr")
  
  file_name_sfh <- paste0("EnergyProportions_",bezirk_number,"_",bezirk,"SFH.csv")
  file_name_mfh <- paste0("EnergyProportions_",bezirk_number,"_",bezirk,"MFH.csv")
    
  write.csv2(EPT_SFH_lin,file=paste0(path_to_file , file_name_sfh) , row.names = FALSE)
  write.csv2(EPT_MFH_lin,file=paste0(path_to_file , file_name_mfh) , row.names = FALSE)
