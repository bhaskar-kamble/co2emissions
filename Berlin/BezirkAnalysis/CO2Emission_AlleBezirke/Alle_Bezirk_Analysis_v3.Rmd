---
title: "CO$_2$ Emissionen in allen Bezirken"
...
bezirk_list <- c("charlottenburg_wilmersdorf","friedrichshain_kreuzberg",
                 "lichtenberg","marzahn_hellersdorf","mitte","neukoelln",
                 "pankow","reinickendorf","spandau",
                 "steglitz_zehlendorf","tempelhof_schoeneberg","treptow_koepenick")
#bezirk <- b_list[3] #"lichtenberg"
```
...
mfh_co2_2018 <- NULL
all_co2_2018 <- NULL
sfh_co2_rel_2018 <- NULL
mfh_co2_rel_2018 <- NULL
all_co2_rel_2018 <- NULL
for (bezirk in bezirk_list) {
  ...
  sfh_co2_2018 <- rbind(sfh_co2_2018 , co2_SFH[co2_SFH$abrechnungsjahr==2018 , ] )
  mfh_co2_2018 <- rbind(mfh_co2_2018 , co2_MFH[co2_MFH$abrechnungsjahr==2018 , ] )
  all_co2_2018 <- rbind(all_co2_2018 , co2_ALL[co2_ALL$abrechnungsjahr==2018 , ] )
  sfh_co2_rel_2018 <- rbind(sfh_co2_rel_2018 , co2_SFH_rel[co2_SFH_rel$abrechnungsjahr==2018 , ] )
  mfh_co2_rel_2018 <- rbind(mfh_co2_rel_2018 , co2_MFH_rel[co2_MFH_rel$abrechnungsjahr==2018 , ] )
  all_co2_rel_2018 <- rbind(all_co2_rel_2018 , co2_ALL_rel[co2_ALL_rel$abrechnungsjahr==2018 , ] )
}
sfh_co2_2018$bezirk <- bezirk_list
mfh_co2_2018$bezirk <- bezirk_list
all_co2_2018$bezirk <- bezirk_list
sfh_co2_rel_2018$bezirk <- bezirk_list
mfh_co2_rel_2018$bezirk <- bezirk_list
all_co2_rel_2018$bezirk <- bezirk_list
```
...
```{r}
#function for rounding off everything to two digits:
round_off <- function(obj,drop_cols) {
  columns_to_round_off <- names(obj)[!(names(obj) %in% drop_cols)]
  for (colum in columns_to_round_off) {
    obj[[colum]] <- round(obj[[colum]] , 2)
  }
  return(obj)
}
```
...
t2 <- ttheme_default(base_size = 12,      colhead=list(fg_params=list(hjust=0,x=0.01))) 
header02 <- tableGrob(table01[1, 1:2],rows=NULL, cols=c(" " , "CO2 Emission in 1.000 t"),theme=t2) 
jn <- gtable_combine(header02[1,],jn,along=2)
jn$layout[1:4 , c("r","l")] <- list(c(1,2),c(1,7)) # Wtf is the logic for 1:4 ???



t3 <- ttheme_default(base_size = 12,      colhead=list(fg_params=list(hjust=0,x=0.01))) 
header03 <- tableGrob(table01[1, 1],rows=NULL, cols=c("1-2 Familiengebäude"),theme=t3) 
jn <- gtable_combine(header03[1,],jn,along=2)
jn$layout[1:2 , c("r","l")] <- list(1, 7)

t4 <- ttheme_default(base_size = 12,      colhead=list(fg_params=list(hjust=0,x=0.01))) 
header04 <- tableGrob(table01[1, 1],rows=NULL, cols=c("CO2 Emissionen aus Beheizung von Wohngebäuden, Berliner Stadtbezirke, 2018"),theme=t4) 
jn <- gtable_combine(header04[1,],jn,along=2)
#jn$widths <- rep(5,7)
jn$widths <- c(7,5,5,5,5,5,5)
jn$layout[1:2 , c("r","l")] <- list(1, 7)
